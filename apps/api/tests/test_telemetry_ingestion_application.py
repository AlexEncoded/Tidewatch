...
def test_turbidity_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_turbidity_snapshot(
        "buoy-1",
        {
            "turbidity_ntu": 12.6,
            "sensor_channel": "B",
            "device_id": None,
            "sensor_id": "turbidity-b",
    ...
    assert snapshot.turbidity_ntu == 12.6
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at
...
def test_ph_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_ph_snapshot(
        "buoy-1",
        {
            "ph": 8.15,
            "sensor_channel": "B",
    ...
    assert snapshot.ph == 8.15
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at


def test_conductivity_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_conductivity_snapshot(
        "buoy-1",
        {
            "conductivity_us_cm": 52300.0,
            "sensor_channel": "B",
            "device_id": None,
            "sensor_id": "conductivity-b",
    ...
    assert snapshot.conductivity_us_cm == 52300.0
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at


def test_chlorophyll_a_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_chlorophyll_a_snapshot(
        "buoy-1",
        {
            "chlorophyll_a_ug_l": 4.7,
            "sensor_channel": "B",
            "device_id": None,
            "sensor_id": "chlorophyll-b",
    ...
    assert snapshot.chlorophyll_a_ug_l == 4.7
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at
...
def test_humidity_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_humidity_snapshot(
        "buoy-1",
        {
            "humidity_percent": 72.5,
            "sensor_channel": "B",
            "device_id": None,
            "sensor_id": "humidity-b",
            "firmware_version": "2.0",
            "quality": "good",
            "measured_at": measured_at,
        },
    ...
    assert snapshot.humidity_percent == 72.5
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at


def test_air_temperature_payload_maps_to_domain_snapshot_with_batch_provenance() -> None:
    measured_at = datetime(2026, 9, 25, tzinfo=timezone.utc)
    snapshot = build_air_temperature_snapshot(
        "buoy-1",
        {
            "air_temperature_celsius": 19.3,
            "sensor_channel": "B",
            "device_id": None,
            "sensor_id": "air-temp-b",
            "firmware_version": "2.1",
            "quality": "good",
            "measured_at": measured_at,
        },
    ...
    assert snapshot.air_temperature_celsius == 19.3
    assert snapshot.sensor_channel == "B"
    assert snapshot.device_id == "unit-b"
    assert snapshot.measured_at == measured_at
