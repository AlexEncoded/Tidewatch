...
def test_batch_chlorophyll_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Chlorophyll Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "chlorophyll-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={
            "device_id": "chlorophyll-unit",
            "chlorophyll_a": [{"chlorophyll_a_ug_l": 3}],
        },
    )

    assert response.status_code == 202
    assert client.get(f"/api/v1/buoys/{buoy_id}/chlorophyll-a").json()[0]["device_id"] == "chlorophyll-unit"


def test_batch_rainfall_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Rain Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "rain-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={"device_id": "rain-unit", "rainfall": [{"rainfall_mm_h": 2.5}]},
    )

    assert response.status_code == 202
    assert client.get(f"/api/v1/buoys/{buoy_id}/rainfall").json()[0]["device_id"] == "rain-unit"


def test_batch_humidity_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Humidity Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "humidity-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={"device_id": "humidity-unit", "humidity": [{"humidity_percent": 60}]},
    )
...
def test_batch_atmospheric_pressure_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Atmospheric Pressure Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "atmospheric-pressure-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={
            "device_id": "atmospheric-pressure-unit",
            "atmospheric_pressure": [{"atmospheric_pressure_kpa": 101.3}],
        },
    )

    assert response.status_code == 202
    assert client.get(f"/api/v1/buoys/{buoy_id}/atmospheric-pressure").json()[0]["device_id"] == "atmospheric-pressure-unit"


def test_batch_acoustic_altimeter_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Altimeter Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "altimeter-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={
            "device_id": "altimeter-unit",
            "acoustic_altimeter": [{"depth_meters": 25}],
        },
    )

    assert response.status_code == 202
    assert client.get(f"/api/v1/buoys/{buoy_id}/acoustic-altimeter").json()[0]["device_id"] == "altimeter-unit"


def test_batch_underwater_acoustic_keeps_originating_device(client):
    buoy_id = client.post("/api/v1/buoys", json={"name": "Underwater Acoustic Device"}).json()["id"]
    client.post(
        f"/api/v1/buoys/{buoy_id}/devices",
        json={"device_id": "underwater-acoustic-unit", "sensor_channel": "A"},
    )

    response = client.post(
        f"/api/v1/buoys/{buoy_id}/telemetry",
        json={
            "device_id": "underwater-acoustic-unit",
            "underwater_acoustic": [{"echo_intensity_db": -42}],
        },
    )

    assert response.status_code == 202
    assert client.get(f"/api/v1/buoys/{buoy_id}/underwater-acoustic").json()[0]["device_id"] == "underwater-acoustic-unit"
